angular.module('NUSTalk.controllers',['firebase'])
	...
	.controller('ModuleController', function($scope, ModuleService, $ionicModal, $http, User, ChatService, $state, $ionicScrollDelegate, Firebase){
		...
		$scope.refresh = function(){
			...
			var ChatsRef = firebase.database().ref('chats/' + ModuleService.getCurrentModule().CourseCode + '/' + User.getUserId());
			
			ChatsRef.on('child_added', function(data) {
				console.log('chat started with '+ data.val().Name);
				var messagesRef = firebase.database().ref('chats/' + ModuleService.getCurrentModule().CourseCode + '/' + User.getUserId() + '/' + data.key);
	...
	.controller('ChatDetailsController', function($scope, $state, ChatService, Firebase, User, $ionicScrollDelegate, $timeout, $rootScope, ModuleService, $cordovaImagePicker){
		...
		$scope.sendMessage = function(msg){
			...
			console.log(data);
			var threadsRef = firebase.database().ref('threads' + '/' + ModuleService.getCurrentModule().CourseCode);

			var flag = 0;
			var currentThread = {};
			threadsRef.once('value', function(snapshot){
				snapshot.forEach(function(childSnapshot){
					var members = childSnapshot.val().members;
					members.forEach(function(member){
						if(member.key == User.getUserId || member.key == ChatService.getOtherUser.UserID){
							flag++;
							if(flag == 2){
								console.log('Chat already exists');
								currentThread = childSnapshot.key;
								break;
							}
						}
					})									
				...
				if(flag != 2){
					...
					threadsRef.push({
						...
						'members':{
							'0': User.getUserId(),
							'1': ChatService.getOtherUser().UserID
						}
					})
					threadsRef.on('child_added', function(data) {
						currentThread = data.key;
					});
				}
			...
			ChatsRef.child(ChatService.getOtherUser().UserID).child(User.getUserId()).child('Name').set(User.getUserName());
			ChatsRef.child(ChatService.getOtherUser().UserID).child(User.getUserId()).push(data);

			ChatsRef.child(User.getUserId()).child(ChatService.getOtherUser().UserID).child('Name').set(ChatService.getOtherUser().Name);
			ChatsRef.child(User.getUserId()).child(ChatService.getOtherUser().UserID).push(data);
		...
		$scope.messages = [];
			...
			var messagesRef = firebase.database().ref('chats/' + ModuleService.getCurrentModule().CourseCode + '/' + User.getUserId() + '/' + ChatService.getOtherUser().UserID);
